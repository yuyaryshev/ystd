...
export function linearDataInterpolation<T extends AnimatedDataItem>(items: T[], timePos: number = new Date().getTime(), timeScale: number = 1): T {
    ...
    if (items.length < 2) throw new Error(`CODE00000191 Can only animate if have more than 2 items in array`);
    ...
    throw new Error(`CODE00000192 Animated data internal error - should be unreachable!`);
}
