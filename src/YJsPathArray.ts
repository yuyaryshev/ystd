...
export class YJsPathArray<T = any> {
    ...
    expectOne(errorMessage: string = `CODE00000218 YJsPath expects exactly one item as output`): T {
    ...
    expectOneOrUndefined(errorMessage: string = `CODE00000219 YJsPath expects exactly one item as output`): T | undefined {
        if (this.items.length > 1) {
            throw new Error(errorMessage);
        }
        return this.items[0];
