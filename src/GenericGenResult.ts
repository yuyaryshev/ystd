...
//                 throw new Error(`CODE00000450 GenResult can contain plain-objects only!`);
//             }
//             return "object";
//         case "string":
//             return tp;
//     }
//
//     throw new Error(`CODE00000451 Unexpected ResultValueType!`);
// }
//
...
//         throw new Error(`CODE000004520 ERROR GenResult type mismatch! ${aType} !== ${bType} `);
//     }
//     const r = aType || bType;
//     if (r === undefined) {
//         throw new Error(`CODE000004530 ERROR GenResult both arguments are undefined!`);
//     }
//     return r;
// }
...
export function appendGenResults<T>(targetGenResult: T, ...genResults: T[]): T {
    if (Array.isArray(targetGenResult)) {
        throw Error(`CODE00000454 appendGenResults expects an array of objects as targetGenResult, but got Array of Arrays!`);
    }

    for (const sourceGenResult of genResults) {
        for (let k in sourceGenResult) {
            if (Array.isArray(sourceGenResult)) {
                throw Error(`CODE00000455 appendGenResults expects an array of objects as genResults, but got Array of Arrays!`);
            }
            ...
            if (sourceGenResult[k] === undefined) {
                ...
                if (!Array.isArray(targetGenResult[k])) {
                    if (targetGenResult[k] !== sourceGenResult[k]) {
                        throw Error(
                            `CODE00000456 mergeGenResults can only merge array fields! But field '${k}' is not an array and has different values in different sources! Field value: ${JSON.stringify(
                    ...
                    for (const valueToAdd of (sourceGenResult as any)[k]) {
                        if (Array.isArray(valueToAdd)) {
                            throw new Error(`CODE00000457 Aggregated value can't contain another array as an item!`);
                        }

                        if ((targetGenResult as any)[k].includes((targetGenResult as any)[k])) {
                            throw new Error(`CODE00000458 Can't add dublicate values!`);
                        }
                    }
                    ...
                    try {
                        ...
                        console.trace(`CODE00000459 appendGenResults failed!`, e);
