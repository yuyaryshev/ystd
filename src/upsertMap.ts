...
export function lookupMap<K, V>(map: Map<K, V>, key: K, cpl?: string, mapName?: string): V {
    ...
    throw new Error(`${cpl || "CODE00000474"} Expected key '${key}' not found in map ${mapName || ""}`.trim());
}
...
export function optLookupMap<K, V>(map: Map<K, V>, key: K | undefined, cpl?: string, mapName?: string): V | undefined {
    ...
    const v = map.get(key);
    if (v !== undefined) return v;
    throw new Error(`${cpl || "CODE00000475"} Expected key '${key}' not found in map ${mapName || ""}`.trim());
}
